...
export function executionLivePresentation(execution) {
  ...
  if (control.previewState === "REPAIRING" && ["QUEUED", "PREPARING", "RUNNING", "VALIDATING", "WAITING_APPROVAL"].includes(status)) {
    return { tone: "active", title: "Corrigindo ambiente", subtitle: "A falha do ambiente foi detectada. Esta é uma nova tentativa de correção; as etapas abaixo mostram somente o ciclo atual.", icon: "running" };
  }
  if (control.previewState === "WAITING_IMPLEMENTATION" && ["QUEUED", "PREPARING", "RUNNING", "VALIDATING", "WAITING_APPROVAL"].includes(status)) {
    return { tone: "active", title: "IA aplicando ajuste", subtitle: "A nova interação está sendo aplicada. Depois disso, o ambiente será republicado e validado novamente.", icon: "running" };
  }
  if (control.awaitingEnvironment) {
    return {
      tone: control.previewState === "FAILED" ? "failed" : "active",
      title: control.environmentStatus ?? "Preparando ambiente",
      subtitle: control.previewState === "FAILED"
        ? "A publicação do ambiente falhou. Quando a correção automática começar, este acompanhamento será reiniciado e mostrará somente o novo ciclo."
        : "A implementação já terminou, mas o ambiente ainda está sendo construído e validado.",
      icon: control.previewState === "FAILED" ? "failed" : "running",
    };
  }
  if (status === "AWAITING_CLIENT") {
    return { tone: "waiting", title: "Aguardando cliente", subtitle: "O ambiente está pronto. Teste o resultado e peça novos ajustes pelo chat quando quiser.", icon: "ready" };
  }
  if (status === "SUCCEEDED") {
