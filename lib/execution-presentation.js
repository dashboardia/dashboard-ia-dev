...
export function executionLivePresentation(execution) {
  ...
  if (control.awaitingEnvironment) {
    return { tone: "active", title: "Preparando ambiente", subtitle: "A IA concluiu a etapa e está construindo a versão navegável para sua validação.", icon: "running" };
  }
  if (control.awaitingPreviewRepairConsent) {
    return { tone: "waiting", title: "Aguardando sua decisão", subtitle: "As correções automáticas não foram suficientes. Confirme se deseja continuar tentando com a IA nesta mesma execução.", icon: "paused" };
  }
