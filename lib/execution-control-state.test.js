...
import { executionControlState, executionPreviewState } from "./execution-control-state.js";
...
test("marca ambiente anterior como aguardando implementação durante nova interação", () => {
  const current = execution({ status: "RUNNING", previewEnvironment: { status: "READY", url: "https://old.example.test" } });
  assert.equal(executionPreviewState(current), "WAITING_IMPLEMENTATION");
  assert.equal(executionControlState(current).interactionAvailable, false);
});
