...
function catalogPrice(value) {
  return Number.isFinite(Number(value)) && value != null ? Number(value) : Number.MAX_SAFE_INTEGER;
}

export function sortBillingPlans(plans) {
  return [...plans].sort((left, right) =>
    catalogPrice(left.priceCents) - catalogPrice(right.priceCents)
    || Math.max(0, left.includedCredits || 0) - Math.max(0, right.includedCredits || 0)
    || left.name.localeCompare(right.name, "pt-BR"));
}

export function sortCreditPacks(packs) {
  return [...packs].sort((left, right) =>
    catalogPrice(left.priceCents) - catalogPrice(right.priceCents)
    || Math.max(0, left.credits || 0) - Math.max(0, right.credits || 0)
    || left.name.localeCompare(right.name, "pt-BR"));
}
...
export async function listBillingPlans(database = db, { includeInactive = true, publicOnly = false } = {}) {
  if (!database?.billingPlanCatalog?.findMany) {
    return sortBillingPlans(Object.values(DEFAULT_BILLING_PLANS)
      .filter((plan) => includeInactive || plan.active)
      .filter((plan) => !publicOnly || plan.public)
      .map(normalizedPlan));
  }
  const plans = await database.billingPlanCatalog.findMany({
    where: {
      ...(!includeInactive ? { active: true } : {}),
      ...(publicOnly ? { public: true } : {}),
    },
  });
  return sortBillingPlans(plans.map(normalizedPlan));
}
...
export async function listCreditPacks(database = db, { includeInactive = true, publicOnly = false } = {}) {
  if (!database?.billingCreditPackCatalog?.findMany) {
    return sortCreditPacks(getCreditPacks()
      .filter((pack) => includeInactive || pack.active)
      .filter((pack) => !publicOnly || pack.public)
      .map(normalizedPlan));
  }
  const packs = await database.billingCreditPackCatalog.findMany({
    where: {
      ...(!includeInactive ? { active: true } : {}),
      ...(publicOnly ? { public: true } : {}),
    },
  });
  return sortCreditPacks(packs.map(normalizedPlan));
}
