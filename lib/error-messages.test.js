...
describe("error messages", () => {
  ...
  it("explica quando o ciclo controlado de três tentativas termina", () => {
    const result = explainError("As 3 tentativas automáticas atingiram o limite total de segurança de 120000 tokens medidos.");
    expect(result.title).toBe("As três tentativas automáticas foram utilizadas");
    expect(result.action).toContain("autorize um novo ciclo");
  });

  it("não apresenta falha de inicialização como entrega concluída", () => {
    const result = explainError("A validação runtime falhou; o comando de preview foi encerrado antes de iniciar a aplicação");
    expect(result.title).toBe("A aplicação não iniciou corretamente");
    expect(result.message).toContain("não respondeu");
  });
