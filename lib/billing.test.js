...
describe("credit settlement", () => {
  it("reconcilia dívida inflada por reservas paralelas usando concessões e consumo do ciclo", async () => {
    ...
    expect(database.creditBucket.update).toHaveBeenNthCalledWith(1, { where: { id: "monthly" }, data: { remaining: 0 } });
    expect(database.creditBucket.update).toHaveBeenNthCalledWith(2, { where: { id: "pack" }, data: { remaining: 700 } });
