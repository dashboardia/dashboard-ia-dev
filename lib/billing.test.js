...
import { activatePlanUpgrade, assertAiModelAccess, assertCanCreateProject, billingAccessIsActive, chargeFixedProjectCredits, claimTrialOrganization, ensureBillingAccount, getExecutionCreditBudget, getProjectBillingContext, reconcileCurrentCycleCredits, settleExecutionCredits } from "./billing";
...
describe("billing access", () => {
  ...
  it("libera uso por créditos comprados sem assinatura e mantém os limites gratuitos", async () => {
    const database = {
      project: { findUniqueOrThrow: vi.fn().mockResolvedValue({ createdBy: { id: "user-1", githubLogin: "camila", globalRole: "USER" } }) },
      billingAccount: { findUnique: vi.fn().mockResolvedValue({ id: "account-1", ownerUserId: "user-1", plan: "STUDIO", status: "EXPIRED", creditDebt: 0 }) },
      creditBucket: { findMany: vi.fn().mockResolvedValue([{ remaining: 1_000, reserved: 0 }]) },
    };

    const context = await getProjectBillingContext("project-1", database);

    expect(context.creditOnly).toBe(true);
    expect(context.plan.code).toBe("TRIAL");
    expect(context.availableCredits).toBe(1_000);
    expect(assertAiModelAccess(context, "gpt-5.6-luna")).toBe(context);
    expect(() => assertAiModelAccess(context, "gpt-5.6-terra")).toThrow(/Studio.*Luna/);
  });
});
