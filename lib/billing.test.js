...
describe("credit settlement", () => {
  ...
  it("nunca consome além da reserva nem cria dívida para o cliente", async () => {
    const transaction = {
      executionCreditReservation: {
    ...
    expect(transaction.creditBucket.update).toHaveBeenCalledTimes(1);
    expect(transaction.creditBucket.update).toHaveBeenCalledWith({
      where: { id: "bucket-1" },
      data: { reserved: { decrement: 700 }, remaining: { decrement: 700 } },
    });
    expect(transaction.creditBucket.findMany).not.toHaveBeenCalled();
    ...
    expect(transaction.executionCreditReservation.update).toHaveBeenCalledWith({
      ...
      data: expect.objectContaining({ consumedCredits: 700, uncoveredCredits: 1_194 }),
    });
  });
});
