...
import { activatePlanUpgrade, assertCanCreateProject, billingAccessIsActive, chargeFixedProjectCredits, claimTrialOrganization, ensureBillingAccount, getExecutionCreditBudget, reconcileCurrentCycleCredits, settleExecutionCredits } from "./billing";
import { BILLING_PLANS, CREDIT_PACKS, executionReservationCredits, getCreditPacks, planChangeKind } from "./billing-plans";
...
describe("credit settlement", () => {
  it("soma os créditos do upgrade sem encerrar nem substituir o saldo atual", async () => {
    ...
    expect(transaction.billingAccount.update).toHaveBeenCalledWith({
      where: { id: "account-1" },
      data: { plan: "AGENCY", pendingPlan: null, status: "ACTIVE", cancelAtPeriodEnd: false, providerSubscriptionId: "sub_1" },
    });
