...
import { applyDetectedRuntime, applyWorkingDirectory, detectProjectRuntime, detectRepositoryRuntime, detectedRuntimeReplacesConfiguration, detectWorkspaceProjectRuntime, environmentRuntimeConfiguration, mavenBuildCommandInRepository } from "./project-runtime";
...
describe("project runtime detection", () => {
  ...
  it("não executa npm só porque existe um package.json auxiliar sem configuração Node", () => {
    const result = detectRepositoryRuntime({
      fileNames: ["package.json", "pom.xml", "src/main/java/App.java"],
      contents: {
        "package.json": JSON.stringify({ name: "ferramentas-do-repositorio" }),
        "pom.xml": "<project><properties><java.version>17</java.version></properties></project>",
      },
    });

    expect(result.runtime).toBe("JAVA_MAVEN_17");
    expect(result.commands.installCommand).toBeNull();
    expect(result.commands.buildCommand).toBe("mvn -B -DskipTests package");
  });
  ...
  it("não reutiliza comandos salvos de outra versão ao criar um ambiente", () => {
    const saved = {
      workingDirectory: ".",
    ...
    const detected = detectRepositoryRuntime({
      fileNames: ["index.php", "README.md"],
      contents: {},
    });

    expect(environmentRuntimeConfiguration(saved, detected)).toMatchObject({
      workingDirectory: ".",
      installCommand: null,
      buildCommand: null,
  ...
  it("detecta o diretório de um projeto Maven fora da raiz", () => {
    ...
    expect(result.commands.previewCommand).toBe("(cd sistema-web && __MAVEN_WAR__)");
    expect(result.commands.previewPort).toBe(8080);
  });
