...
import { githubPullRequestState, githubWebhookUrl, isGitHubWebhookConfirmed, verifyGitHubWebhookSignature } from "./webhooks";
...
describe("isGitHubWebhookConfirmed", () => {
  it("reconhece webhook criado manualmente depois de uma entrega assinada", () => {
    expect(isGitHubWebhookConfirmed({ githubWebhookId: null, githubWebhookAt: new Date() })).toBe(true);
  });

  it("mantem pendente quando nenhuma configuracao ou entrega foi confirmada", () => {
    expect(isGitHubWebhookConfirmed({ githubWebhookId: null, githubWebhookAt: null })).toBe(false);
  });
});
