...
export const ASAAS_CHECKOUT_EXPIRATION_MINUTES = 10;
...
function callbackUrls({ checkoutKind, orderId, returnTo = null }) {
  ...
  return {
    successUrl: `${origin}/billing?checkout=${checkout}&order=${order}${returnQuery}`,
    cancelUrl: `${origin}/billing?checkout=cancelled${returnQuery}`,
    expiredUrl: `${origin}/billing?checkout=expired${returnQuery}`,
  };
}
...
export async function createPlanCheckout({ orderId, plan, customerId = null, customerData = null, returnTo = null }) {
  const result = await asaasRequest("/checkouts", {
    ...
    body: JSON.stringify({
      ...
      callback: callbackUrls({ checkoutKind: "PLAN", orderId, returnTo }),
...
export async function createCreditPackCheckout({ orderId, pack, customerId = null, customerData = null, returnTo = null }) {
  const result = await asaasRequest("/checkouts", {
    method: "POST",
    body: JSON.stringify({
      ...
      minutesToExpire: ASAAS_CHECKOUT_EXPIRATION_MINUTES,
      externalReference: orderId,
      callback: callbackUrls({ checkoutKind: "CREDIT_PACK", orderId, returnTo }),
      ...checkoutCustomerFields({ customerId, customerData }),
...
async function customerIdFromEmail(email) {
  ...
  if (customers.length > 1) throw new Error("Mais de um cliente com este e-mail foi encontrado no Asaas.");
  return customers[0]?.id || null;
}

export async function resolveAsaasCustomerId({ customerId, customerEmail, checkouts = [] }) {
  ...
  return customerIdFromEmail(customerEmail);
}
