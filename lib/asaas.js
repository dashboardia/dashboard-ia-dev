...
function checkoutCustomerFields({ customerId, customerData } = {}) {
  ...
  const normalized = Object.fromEntries(Object.entries(customerData || {}).filter(([, value]) => typeof value === "string" && value.trim()));
  return Object.keys(normalized).length ? { customerData: normalized } : {};
}
