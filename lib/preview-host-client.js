...
export async function syncDashboardiaPreview(database, environment) {
  ...
  const now = new Date();
  const remoteReadyAt = parsedRemoteDate(remote.readyAt);
...
export async function syncExecutionPreviewForPresentation(database, execution) {
  ...
  if (preview.status === "READY") {
    ...
    if (currentReady) return execution;
  }

  try {
    const previewEnvironment = await syncDashboardiaPreview(database, preview);
