...
export const MAX_ATTACHMENTS_TOTAL_BYTES = 12_000_000;
...
export function validateAttachmentFiles(files) {
  ...
  if (selected.length > MAX_MESSAGE_ATTACHMENTS) {
    const error = new Error(`Envie no máximo ${MAX_MESSAGE_ATTACHMENTS} arquivos por mensagem.`);
    error.status = 400;
  ...
  return selected.map((file) => {
    ...
    if (size > MAX_ATTACHMENT_BYTES) {
      const error = new Error(`${name}: cada arquivo pode ter até 5 MB.`);
      error.status = 400;
      error.code = "INVALID_ATTACHMENT";
      throw error;
    }
    ...
    if (totalBytes > MAX_ATTACHMENTS_TOTAL_BYTES) {
      const error = new Error("Os anexos da mensagem podem somar até 12 MB.");
      error.status = 400;
      error.code = "INVALID_ATTACHMENT";
      throw error;
    }
