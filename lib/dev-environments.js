...
export async function chargeReadyDevEnvironment(database, environment, now = new Date()) {
  ...
  return database.$transaction(async (transaction) => {
    const claimed = await transaction.devEnvironment.updateMany({
      where: { id: environment.id, status: "READY", creditChargedAt: null },
      data: { creditChargedAt: now },
    });
    if (claimed.count !== 1) return false;
    const charge = await settleFixedCreditsInTransaction(
      transaction,
      reservation,
      `Ambiente publicado com sucesso · ${environment.branchName}`,
    );
...
export async function stopDevEnvironment(database, environment) {
  ...
  const stopped = await database.devEnvironment.update({
