...
import { demandInputSchema, demandUpdateSchema, projectDeleteSchema, projectInputSchema, projectUpdateSchema } from "./validation";
...
describe("projectDeleteSchema", () => {
  it("exige confirmação não vazia", () => {
    expect(projectDeleteSchema.parse({ confirmation: "Dashboard IA" }).confirmation).toBe("Dashboard IA");
    expect(() => projectDeleteSchema.parse({ confirmation: "" })).toThrow();
  });
});
