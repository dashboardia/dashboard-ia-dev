...
import { billingCheckoutSchema, billingPlanCatalogSchema, demandInputSchema, demandUpdateSchema, globalSettingsSchema, projectInputSchema, projectUpdateSchema } from "./validation";

describe("billingCheckoutSchema", () => {
  it("aceita clientes antigos que ainda enviam returnTo nulo", () => {
    expect(billingCheckoutSchema.parse({ kind: "CREDIT_PACK", pack: "CREDITS_100", returnTo: null })).toEqual({
      kind: "CREDIT_PACK",
      pack: "CREDITS_100",
      returnTo: undefined,
    });
  ...
  it("preserva somente o retorno seguro para uma execução", () => {
    expect(billingCheckoutSchema.parse({ kind: "PLAN", plan: "PRO", returnTo: "/executions/abc_123" }).returnTo).toBe("/executions/abc_123");
    expect(() => billingCheckoutSchema.parse({ kind: "PLAN", plan: "PRO", returnTo: "/billing" })).toThrow();
  });
});
