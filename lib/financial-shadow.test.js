...
import { calculateDisplayedExecutionCredits, calculateFinancialSnapshot, calculateLiveUsageCredits, MODEL_PRICING } from "./financial-shadow";
...
describe("execution credit presentation", () => {
  it("does not let an empty reservation hide measured consumption", () => {
    const credits = calculateDisplayedExecutionCredits({
      reservationConsumedCredits: 0,
      snapshotConsumedCredits: 84,
      model: "gpt-5.6-luna",
      inputTokens: 1_033_174,
      outputTokens: 0,
      settings: defaultGlobalSettings,
    });

    expect(credits).toBeGreaterThanOrEqual(84);
  });
});
