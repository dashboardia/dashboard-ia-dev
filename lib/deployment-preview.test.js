...
import { configuredProjectPreview, expireStaleDeploymentPreview, findDeploymentPreview, inspectDeploymentPreview } from "./deployment-preview";

const publicResolver = async () => [{ address: "203.0.113.10", family: 4 }];

describe("configuredProjectPreview", () => {
  it("usa a aplicação publicada do projeto como preview navegável", () => {
    expect(configuredProjectPreview("https://app-demo.onrender.com")).toMatchObject({
      state: "AVAILABLE",
      url: "https://app-demo.onrender.com/",
      provider: "Render",
      source: "project_url",
    });
  });
  ...
  it("recusa URL sem HTTPS", () => {
    expect(configuredProjectPreview("http://app-demo.example.com")).toBeNull();
  });
});
