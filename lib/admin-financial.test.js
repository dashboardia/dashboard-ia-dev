...
describe("admin financial dashboard", () => {
  ...
  it("agrupa receita confirmada e custos medidos pelo mês real de cada evento", () => {
    const series = buildClientFinancialMonthlySeries([
      ...
      { calculatedAt: new Date("2026-08-03T12:00:00Z"), aiCostUsdMicros: 100_000, usdToBrlCents: 500, totalInternalCostBrlCents: 900 },
    ]);

    expect(series).toEqual([
      { month: "2026-07", paidBrlCents: 10_000, aiCostBrlCents: 0, totalInternalCostBrlCents: 0, executions: 0, resultBrlCents: 10_000 },
      { month: "2026-08", paidBrlCents: 5_000, aiCostBrlCents: 50, totalInternalCostBrlCents: 900, executions: 1, resultBrlCents: 4_100 },
    ]);
  });
});
