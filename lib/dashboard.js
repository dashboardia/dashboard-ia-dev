import { db } from "./db";
import { executionControlState } from "./execution-control-state";
import { syncExecutionPreviewForPresentation } from "./preview-host-client";
...
export async function getDashboardData(user) {
  ...
  const [projects, connectedProjects, recentDemands, activeDemands, executionsToday, successfulToday, healthCheckCount, healthyHealthCheckCount, recentHealthChecks, activeExecutions] = await Promise.all([
    ...
    db.execution.findMany({
      where: {
        ...
        status: { in: ["QUEUED", "PREPARING", "RUNNING", "VALIDATING", "WAITING_APPROVAL", "AWAITING_CLIENT", "STOPPED"] },
        cancelRequestedAt: null,
        closedAt: null,
      },
      include: {
        demand: { select: { id: true, title: true, type: true, project: { select: { name: true } } } },
  ...
  const synchronizedActiveExecutions = await Promise.all(
    activeExecutions.map((execution) => syncExecutionPreviewForPresentation(db, execution)),
  );
  ...
  return {
    ...
    activeWork: synchronizedActiveExecutions.map((execution) => {
      ...
      return {
        ...
        status: control.displayStatus,
        tone: controlTone[control.displayTone] ?? "queued",
