...
export async function getDashboardData(user) {
  ...
  const [projects, recentDemands, activeDemands, executionsToday, successfulToday, healthChecks] = await Promise.all([
    db.project.findMany({
    ...
    db.healthCheck.findMany({ where: { project: access, checkedAt: { gte: lastDay } }, orderBy: { checkedAt: "asc" }, take: 240 }),
  ]);

  const healthyChecks = healthChecks.filter((check) => check.status === "HEALTHY").length;
  const availability = healthChecks.length ? (healthyChecks / healthChecks.length) * 100 : null;
  const chartSource = healthChecks.slice(-24);
  ...
  return {
    ...
    health: {
      availability: availability === null ? "—" : `${availability.toFixed(1).replace(".", ",")}%`,
      title: healthChecks.length ? (availability >= 99 ? "Operação estável" : "Operação requer atenção") : "Monitoramento pendente",
      subtitle: healthChecks.length ? `Última verificação ${relativeTime(healthChecks.at(-1).checkedAt)}` : "Conecte a URL de produção dos projetos",
