export const FINANCIAL_FORMULA_VERSION = "metered-v2-2026-08-20";
...
export function calculateDisplayedExecutionCredits({
  reservationConsumedCredits,
  snapshotConsumedCredits,
  model,
  inputTokens,
  outputTokens,
  settings,
}) {
  const measuredCredits = calculateLiveUsageCredits({
    model,
    inputTokens,
    outputTokens,
  ...
  return Math.max(
    0,
    Number(reservationConsumedCredits) || 0,
    Number(snapshotConsumedCredits) || 0,
    measuredCredits,
  );
}
...
export function calculateFinancialSnapshot({
  ...
  runStartedAt,
  previousSnapshot,
  visualValidationPerformed = false,
}) {
  ...
  const startedAt = runStartedAt
    ...
    : execution.startedAt ? new Date(execution.startedAt) : endedAt;
  const currentWorkerDurationSeconds = Math.max(0, Math.ceil((endedAt.getTime() - startedAt.getTime()) / 1000));
  const workerDurationSeconds = Math.max(0, Number(previousSnapshot?.workerDurationSeconds) || 0) + currentWorkerDurationSeconds;
  const workerCostBrlCents = hasMeasuredUsage
    ? ceilDivide(workerDurationSeconds * settings.workerCostCentsPerHour, 3_600)
    : 0;
  const visualValidationCostBrlCents = Math.max(0, Number(previousSnapshot?.visualValidationCostBrlCents) || 0)
    + (hasMeasuredUsage && visualValidationPerformed ? settings.visualValidationCostCents : 0);
...
export async function saveFinancialSnapshot(database, options) {
  const { execution, settings } = options;
  const snapshot = calculateFinancialSnapshot(options);
