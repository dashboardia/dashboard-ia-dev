...
export function createSecretCipher(encodedKey, options = {}) {
  const key = decodeEncryptionKey(encodedKey);
  const allowPlaintext = options.allowPlaintext ?? env.NODE_ENV !== "production";
  ...
  function encrypt(value, purpose) {
    ...
    if (!key) {
      if (allowPlaintext) return value;
      throw new Error("TOKEN_ENCRYPTION_KEY não está configurada; o token OAuth não será salvo sem criptografia");
    }
