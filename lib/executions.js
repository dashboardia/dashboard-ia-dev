...
import { isExecutionCreditBlocked } from "./execution-credit-state.js";
...
export async function reopenFailedExecutionForCorrection(executionId, database = db, { now = new Date(), timeoutMinutes = MINIMUM_CONVERSATION_TIMEOUT_MINUTES } = {}) {
  const current = await database.execution.findUnique({
    ...
    select: { id: true, demandId: true, status: true, closedAt: true, error: true },
  });
  ...
  return database.$transaction(async (transaction) => {
    ...
    const creditBlocked = isExecutionCreditBlocked(current.error);
    await transaction.executionMessage.create({
      data: {
        executionId,
        role: "SYSTEM",
        content: creditBlocked
          ? "O processamento foi pausado porque os créditos disponíveis não são suficientes para continuar. Nenhum trabalho foi perdido: adicione créditos e retome esta mesma demanda do ponto em que ela parou."
          : "A execução encontrou uma falha e ficou aberta para correção. Use a opção Corrigir com IA para continuar nesta mesma execução; ela só será encerrada quando você concluir ou após o período de inatividade.",
      },
    });
    return true;
  });
...
export async function createQueuedDemandExecution({ demandData, requestedById, billing = null, allowEmptyRepository = false, auditFactory = null, initialMessage = null }, database = db) {
  for (let attempt = 1; attempt <= MAX_TRANSACTION_ATTEMPTS; attempt += 1) {
    try {
      return await database.$transaction(async (transaction) => {
        ...
        if (initialMessage) {
          await transaction.executionMessage.create({
            data: {
              ...
              attachments: { create: initialMessage.attachments ?? [] },
            },
          });
        }
        if (billing && !billing.bypass) {
        ...
        const auditLogs = auditFactory?.({ demand, execution }) ?? [];
        for (const data of auditLogs) await transaction.auditLog.create({ data });
        return { demand, execution };
      }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
    } catch (error) {
      if (!isTransactionConflict(error) || attempt === MAX_TRANSACTION_ATTEMPTS) throw error;
      await new Promise((resolve) => setTimeout(resolve, attempt * 20));
  ...
  throw new Error("Não foi possível criar e enfileirar a demanda");
}
