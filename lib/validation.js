...
const billingReturnPathSchema = z.string()
  .regex(/^\/executions\/[A-Za-z0-9_-]+$/)
  .nullish()
  .transform((value) => value ?? undefined);

export const billingCheckoutSchema = z.discriminatedUnion("kind", [
  z.object({ kind: z.literal("PLAN"), plan: billingPlanCodeSchema, returnTo: billingReturnPathSchema }),
  z.object({ kind: z.literal("CREDIT_PACK"), pack: billingPlanCodeSchema, returnTo: billingReturnPathSchema }),
]);
