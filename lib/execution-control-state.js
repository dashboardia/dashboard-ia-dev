...
function awaitingEnvironmentStatus(previewState) {
  if (previewState === "FAILED") return "Falha no ambiente";
  if (previewState === "EXPIRED") return "Ambiente expirado";
...
export function executionControlState(execution, preview = execution?.previewEnvironment ?? null) {
  ...
  const displayStatus = execution?.cancelRequestedAt && status !== "CANCELLED"
    ...
    : awaitingEnvironment
      ? awaitingEnvironmentStatus(previewState)
  ...
  else if (status === "FAILED" || (awaitingEnvironment && ["FAILED", "EXPIRED"].includes(previewState))) displayTone = "failed";
