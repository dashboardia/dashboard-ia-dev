...
import { previewRepairConsentRequired } from "./preview-repair-consent.js";
...
export function executionControlState(execution, preview = execution?.previewEnvironment ?? null) {
  ...
  const awaitingPreviewRepairConsent = status === "AWAITING_CLIENT"
    && previewState === "FAILED"
    && previewRepairConsentRequired(preview?.error);
  ...
  const interactionAvailable = !blocked && !creditBlocked && (paused || recoverableFailure || status === "AWAITING_CLIENT");

  const displayStatus = execution?.cancelRequestedAt && status !== "CANCELLED"
    ? "Cancelamento solicitado"
    : creditBlocked
      ? "Aguardando créditos"
      : awaitingPreviewRepairConsent
        ? "Aguardando decisão"
        : awaitingEnvironment
          ? "Preparando ambiente"
  ...
  const canPause = !blocked && !execution?.stopRequestedAt && (processing || awaitingEnvironment);
  ...
  return { status, displayStatus, displayTone, environmentStatus, previewState, previewReady, awaitingEnvironment, awaitingPreviewRepairConsent, interactionAvailable, creditBlocked, canPause, canResume, canCancel, canRestartEnvironment };
}
