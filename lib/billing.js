...
export async function settleExecutionCredits(transaction, { executionId, consumedCredits }) {
  ...
  const chargeableCredits = Math.min(measuredCredits, reservation.reservedCredits);
  let remainingToConsume = chargeableCredits;
  let consumed = 0;
  ...
  const released = Math.max(0, reservation.reservedCredits - chargeableCredits);
  ...
  return transaction.executionCreditReservation.update({
    ...
    data: {
      ...
      uncoveredCredits: Math.max(0, measuredCredits - reservation.reservedCredits),
      settledAt: new Date(),
    },
  });
}
