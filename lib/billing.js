...
import { executionReservationCredits, getBillingPlan, planIsPaid } from "./billing-plans.js";
...
export async function activatePlanUpgrade(transaction, { account, planCode, sourceRef, providerSubscriptionId = null }) {
  const plan = await getBillingPlan(planCode, transaction);
  const now = new Date();
