...
export async function reconcileCurrentCycleCredits(database, account) {
  if (!["STUDIO", "AGENCY"].includes(account?.plan) || account.status !== "ACTIVE") return null;
...
export async function chargeFixedProjectCredits({ projectId, credits, description, metadata }, database = db) {
  ...
  return database.$transaction(async (transaction) => {
    ...
    await transaction.creditTransaction.create({
      data: { accountId: context.account.id, type: "CONSUME", amount: -normalizedCredits, description, metadata },
    });
