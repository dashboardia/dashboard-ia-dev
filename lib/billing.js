...
export async function assertCanCreateProject(user, database = db) {
  ...
  if (!overview.accessActive) throw new BillingAccessError("Seu teste terminou e não há créditos disponíveis. Recarregue via Pix ou escolha um plano para conectar novos projetos.");
...
export async function getProjectBillingContext(projectId, database = db) {
  ...
  if (!availableCredits) {
    throw new BillingAccessError("O teste terminou e não há créditos disponíveis. Recarregue via Pix ou escolha um plano para iniciar novas execuções.");
  }
