...
test("limita globalmente reparos automáticos e consentidos na mesma execução", () => {
  const automatic = { metadata: { automatic: true, aiInvoked: true, previewAiRepair: true } };
  const consented = { metadata: { automatic: false, aiInvoked: true, previewAiRepair: true, previewRepairConsentGranted: true } };
  const logs = [automatic, automatic, consented];
