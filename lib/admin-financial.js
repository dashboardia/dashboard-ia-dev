export function buildClientFinancialRows(accounts, snapshots) {
  const rows = new Map((accounts || []).map((account) => [account.ownerUserId, {
    ...
    plan: account.plan,
    planName: account.planDefinition?.name ?? account.plan,
    ...
    outputTokens: 0,
    aiCostBrlCents: 0,
    adjustedAiCostBrlCents: 0,
    aiSafetyCostBrlCents: 0,
    workerCostBrlCents: 0,
    visualCostBrlCents: 0,
    directOperationalCostBrlCents: 0,
  ...
  for (const snapshot of snapshots || []) {
    ...
    const adjustedAiCostBrlCents = Math.max(measuredAiCostBrlCents, snapshot.adjustedAiCostBrlCents || 0);
    const workerCostBrlCents = Math.max(0, snapshot.workerCostBrlCents || 0);
    const visualCostBrlCents = Math.max(0, snapshot.visualValidationCostBrlCents || 0);
    row.adjustedAiCostBrlCents += adjustedAiCostBrlCents;
    row.aiSafetyCostBrlCents += Math.max(0, adjustedAiCostBrlCents - measuredAiCostBrlCents);
    row.workerCostBrlCents += workerCostBrlCents;
    row.visualCostBrlCents += visualCostBrlCents;
    row.directOperationalCostBrlCents += measuredAiCostBrlCents + workerCostBrlCents + visualCostBrlCents;
...
export function summarizeClientFinancialRows(rows) {
  const summary = (rows || []).reduce((total, row) => ({
    ...
    aiSafetyCostBrlCents: total.aiSafetyCostBrlCents + row.aiSafetyCostBrlCents,
    workerCostBrlCents: total.workerCostBrlCents + row.workerCostBrlCents,
    visualCostBrlCents: total.visualCostBrlCents + row.visualCostBrlCents,
    directOperationalCostBrlCents: total.directOperationalCostBrlCents + row.directOperationalCostBrlCents,
  ...
  }), { paidBrlCents: 0, aiCostBrlCents: 0, aiSafetyCostBrlCents: 0, workerCostBrlCents: 0, visualCostBrlCents: 0, directOperationalCostBrlCents: 0, totalInternalCostBrlCents: 0, executions: 0 });
...
export function buildClientFinancialMonthlySeries(checkouts, snapshots, limit = 12) {
  ...
  const getMonth = (key) => {
    if (!months.has(key)) months.set(key, { month: key, paidBrlCents: 0, aiCostBrlCents: 0, aiSafetyCostBrlCents: 0, workerCostBrlCents: 0, visualCostBrlCents: 0, directOperationalCostBrlCents: 0, totalInternalCostBrlCents: 0, executions: 0 });
  ...
  for (const snapshot of snapshots || []) {
    ...
    const workerCostBrlCents = Math.max(0, snapshot.workerCostBrlCents || 0);
    const visualCostBrlCents = Math.max(0, snapshot.visualValidationCostBrlCents || 0);
    month.aiCostBrlCents += measuredAiCostBrlCents;
    month.aiSafetyCostBrlCents += Math.max(0, adjustedAiCostBrlCents - measuredAiCostBrlCents);
    month.workerCostBrlCents += workerCostBrlCents;
    month.visualCostBrlCents += visualCostBrlCents;
    month.directOperationalCostBrlCents += measuredAiCostBrlCents + workerCostBrlCents + visualCostBrlCents;
  ...
  return [...months.values()].sort((left, right) => left.month.localeCompare(right.month)).slice(-Math.max(1, limit)).map((month) => ({
    ...
    grossMarginPercent: month.paidBrlCents ? Math.round((month.paidBrlCents - month.totalInternalCostBrlCents) * 10_000 / month.paidBrlCents) / 100 : null,
  }));
}
