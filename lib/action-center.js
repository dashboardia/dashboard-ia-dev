...
export async function getActionCenter({ user, client = db, now = new Date() }) {
  ...
  const synchronizedReadyExecutions = await Promise.all(
    readyExecutions.map((execution) => syncExecutionPreviewForPresentation(client, execution)),
  );

  const unhealthyProjects = projects
    .filter((project) => ["DOWN", "DEGRADED"].includes(project.healthChecks[0]?.status))
    .slice(0, RESULT_LIMIT);
  const clientReadyExecutions = synchronizedReadyExecutions
    .filter((execution) => execution.demand.type === "DOCUMENTATION" || executionControlState(execution).previewReady)
    .slice(0, RESULT_LIMIT);
