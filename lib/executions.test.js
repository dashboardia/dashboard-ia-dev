...
describe("execution queue", () => {
  ...
  it("registra anexos da primeira mensagem na mesma transação da execução", async () => {
    const demand = { id: "demand-with-files", aiModel: "gpt-5.6-luna", project: { id: "project-1" } };
    const execution = { id: "execution-with-files", status: "QUEUED" };
    const transaction = {
      demand: { create: vi.fn().mockResolvedValue(demand) },
      execution: { create: vi.fn().mockResolvedValue(execution) },
      executionMessage: { create: vi.fn().mockResolvedValue({ id: "message-1" }) },
      auditLog: { create: vi.fn() },
    };
    const database = transactionDatabase(transaction);
    const attachment = { name: "referencia.png", mimeType: "image/png", sizeBytes: 512, storageKey: "execution-messages/initial/reference.png" };

    await createQueuedDemandExecution({
      demandData: { projectId: "project-1", title: "Nova tela", aiModel: "gpt-5.6-luna" },
      requestedById: "user-1",
      initialMessage: { authorId: "user-1", content: "Use esta referência", attachments: [attachment] },
    }, database);

    expect(transaction.executionMessage.create).toHaveBeenCalledWith({
      data: {
        executionId: execution.id,
        authorId: "user-1",
        role: "USER",
        content: "Use esta referência",
