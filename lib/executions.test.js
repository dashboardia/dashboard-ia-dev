...
import { claimNextExecution, queueDemandExecution, recoverStaleExecutions } from "./executions";
...
describe("stale execution recovery", () => {
  it("sincroniza a demanda ao cancelar, repetir ou encerrar execuções interrompidas", async () => {
    ...
    expect(transaction.execution.updateMany.mock.calls.map(([input]) => input.data.status)).toEqual(["STOPPED", "CANCELLED", "QUEUED", "FAILED"]);
    expect(transaction.demand.update.mock.calls.map(([input]) => input.data.status)).toEqual(["STOPPED", "APPROVED", "QUEUED", "FAILED"]);
...
describe("execution claim", () => {
  it("não reivindica trabalho enquanto o processamento global está pausado", async () => {
    const database = {
      globalSettings: { findUnique: vi.fn().mockResolvedValue({ executionProcessingEnabled: false }) },
      execution: { findFirst: vi.fn(), updateMany: vi.fn() },
    };
    await expect(claimNextExecution("worker-1", database, 5)).resolves.toBeNull();
    expect(database.execution.findFirst).not.toHaveBeenCalled();
  });
});
