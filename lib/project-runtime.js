...
export function detectRepositoryRuntime({ fileNames, contents }) {
  ...
  if (node && node.runtime !== "UNKNOWN") return { ...node, commands: commandsInDirectory(node.commands, packageDirectory) };
  if (python && python.runtime !== "UNKNOWN") return { ...python, commands: commandsInDirectory(python.commands, pythonDirectory) };
  ...
  return { ...fallback, commands: commandsInDirectory(fallback.commands, fallbackDirectory) };
}
