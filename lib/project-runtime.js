...
export function detectProjectRuntime({ fileNames, packageJson, requirements = "", pomXml = "", dotnetProject = "", composerJson }) {
  ...
  if (files.has("pom.xml")) {
    ...
    const javaVersion = [
      /<java\.version>\s*(\d+)/i,
      /<maven\.compiler\.release>\s*(\d+)/i,
      /<maven\.compiler\.source>\s*(\d+)/i,
      /<release>\s*(\d+)/i,
      /<source>\s*(\d+)/i,
    ].map((pattern) => pomXml.match(pattern)?.[1]).find(Boolean);
    const runtime = javaVersion ? `JAVA_MAVEN_${javaVersion}` : "JAVA_MAVEN";
    return { runtime, commands: { installCommand: null, lintCommand: null, testCommand: "mvn -B test", buildCommand: "mvn -B -DskipTests package", previewCommand: spring ? "mvn spring-boot:run -Dspring-boot.run.arguments=--server.port=$PORT" : null, previewPort: spring ? 8080 : null } };
  }
