...
export function buildAgentPrompt(demand, scope = classifyImplementationScope(demand), { businessKnowledge = "", emptyRepository = false, runtimeContext = null } = {}) {
  ...
  return [
    ...
    "Primeiro confirme se o pedido, os arquivos existentes e o contexto disponível permitem construir uma solução funcional e coerente. Resolva decisões técnicas comuns usando os padrões do projeto e defaults seguros de produção; não interrompa o trabalho por preferências opcionais que possam ser inferidas.",
    "Se faltar uma informação realmente indispensável e não existir uma escolha segura que permita entregar ao menos uma primeira versão funcional, não altere arquivos. Retorne exatamente o marcador [DASHBOARDIA_CLARIFICATION_REQUIRED] e, em seguida, faça somente as perguntas objetivas necessárias. Essa resposta abrirá a conversa com o cliente e a execução continuará após ele responder.",
    ...
    "A instalação, o build, o lint, os testes e a inicialização serão executados obrigatoriamente pelo worker após os patches. Não inclua no resumo declarações como 'não executei', 'não foi possível validar' ou qualquer ressalva equivalente sobre essas etapas; o Dashboardia apresentará ao cliente o resultado real da validação separadamente.",
    ...
    "Ao concluir, retorne um resumo objetivo das alterações e dos critérios atendidos. Só mencione uma pendência quando ela exigir uma ação concreta do cliente; nunca transfira ao cliente a responsabilidade de instalar, compilar, testar ou iniciar o projeto.",
    ...approvedKnowledge,
