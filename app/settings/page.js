...
export default async function SettingsPage() {
  ...
  const financialSummary = user.globalRole === "ADMIN" && configuration.database && db.executionFinancialSnapshot
    ? await db.executionFinancialSnapshot.aggregate({
        where: { calculationStatus: "MEASURED" },
        _count: { id: true },
        _sum: { totalInternalCostBrlCents: true, simulatedConsumedCredits: true, simulatedCommercialValueBrlCents: true },
      }).catch(() => null)
    : null;
