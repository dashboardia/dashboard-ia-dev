...
export default async function SettingsPage() {
  ...
  return <AppShell user={user}><div className="section-page narrow-page settings-page"><SectionHeader eyebrow="ADMINISTRAÇÃO" title="Configurações" description="Controle operacional, limites, integrações e parâmetros financeiros da plataforma." /><UserPreferencesForm /><section className="form-card detail-card settings-integration-card"><div className="card-heading"><div><h2>Integrações</h2><p>Serviços e credenciais do ambiente de execução</p></div><Settings size={20} /></div><div className="configuration-list">{items.map(([label, configured, variables, state]) => <div key={label}>{configured ? <CheckCircle2 className="configured" size={18} /> : <CircleAlert className="pending" size={18} />}<span><strong>{label}</strong><small>{variables}</small></span><em>{state ?? (configured ? "Configurado" : "Pendente")}</em></div>)}</div></section>{globalSettings && <><section className="form-card detail-card full-card financial-shadow-summary settings-financial-summary"><div className="card-heading"><div><h2>Calibração financeira</h2><p>Medição interna em modo silencioso; nenhuma cobrança está ativa.</p></div><Coins size={20} /></div><div className="financial-summary-grid"><span><small>Execuções registradas</small><strong>{measuredExecutions}</strong></span><span><small>Custo interno estimado</small><strong>{formatBrlCents(financialSummary?._sum.totalInternalCostBrlCents)}</strong></span><span><small>Créditos simulados</small><strong>{measuredCredits}</strong></span><span><small>Valor comercial simulado</small><strong>{formatBrlCents(financialSummary?._sum.simulatedCommercialValueBrlCents)}</strong></span></div><div className="calibration-progress"><span style={{ width: `${calibrationProgress * 5}%` }} /><small>{calibrationProgress}/20 execuções para a primeira revisão da fórmula</small></div></section><GlobalSettingsForm initialSettings={globalSettings} workerInstances={worker.instances} /></>}</div></AppShell>;
}
