...
export default function EnvironmentsClient({ initialProjects, initialEnvironments }) {
  ...
  return <>
    ...
    <section className="resource-grid environment-grid">
      {environments.map((environment) => { const activity = Array.isArray(environment.activity) ? environment.activity : []; const isActive = ["QUEUED", "BUILDING", "DEPLOYING", "STOPPING"].includes(environment.status); return <article className="resource-card environment-card" key={environment.id}>
