...
export default function Dashboard({ user = null, setupMode = false }) {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [toast, setToast] = useState("");
  const displayName = user?.name ?? "Admin Demo";
  const displayEmail = user?.email ?? "configuracao pendente";
    ...
    <main className="shell">
      <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        ...
        <div className="sidebar-bottom">
          ...
          <div className="user"><span className="user-avatar">{initials}</span><span><strong>{displayName}</strong><small>{displayEmail}</small></span></div>
        </div>
      </aside>
      ...
      <section className="content">
        <header className="topbar">
          ...
          <button className="github-status" onClick={() => action(setupMode ? "GitHub OAuth aguardando configuração" : "GitHub conectado e operacional")}><Github size={18} /><span>{setupMode ? "Configurar GitHub" : "GitHub conectado"}</span><CheckCircle2 size={15} /></button>
        </header>
