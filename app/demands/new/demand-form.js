...
export default function DemandForm({ projects, initialProjectId }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/projects/${selectedProject.id}/branches`, { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        ...
        setForm((current) => {
          const preferred = nextBranches.some((branch) => branch.name === current.baseBranch)
            ? current.baseBranch
      ...
      .catch((fetchError) => {
        ...
        setForm((current) => ({ ...current, baseBranch: selectedProject.defaultBranch }));
  ...
  function change(event) {
    ...
    if (nextProject) {
      setBranches([{ name: nextProject.defaultBranch }]);
      setBranchesLoading(true);
      setBranchError("");
    }
    setForm((current) => projectRequiresLuna
      ? { ...current, projectId: value, baseBranch: nextProject.defaultBranch, aiModel: FREE_PLAN_AI_MODEL }
      : nextProject
        ? { ...current, projectId: value, baseBranch: nextProject.defaultBranch }
  ...
  return (
    <form className="form-card" onSubmit={submit}>
      <div className="form-grid demand-basics">
        ...
        <label><span>Branch base {branchesLoading && <LoaderCircle className="spin branch-loader" size={12} />}</span><select value={form.baseBranch} onChange={(event) => setForm((current) => ({ ...current, baseBranch: event.target.value }))} disabled={!form.projectId || branchesLoading || !branches.length} required>{branchesLoading && <option value={form.baseBranch}>Carregando branches...</option>}{!branchesLoading && branches.map((branch) => <option value={branch.name} key={branch.name}>{branch.name}{branch.protected ? " · protegida" : ""}</option>)}</select><small className={branchError ? "field-warning" : "field-guidance"}>{branchError || (form.projectId ? `${branches.length} branch${branches.length === 1 ? "" : "es"} disponível${branches.length === 1 ? "" : "is"}` : "Selecione primeiro o projeto")}</small></label>
