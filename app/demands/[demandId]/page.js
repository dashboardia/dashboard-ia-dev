  ...
  return (
    <AppShell user={user}>
      <div className="section-page">
        ...
        <section className="form-card detail-card full-card"><div className="card-heading"><div><h2>Execuções</h2><p>Histórico imutável das tentativas</p></div></div><div className="simple-list">{demand.executions.map((execution) => { const error = execution.error ? explainError(execution.error) : null; return <div className="execution-entry" key={execution.id}><strong><Link href={`/executions/${execution.id}`}>{execution.stage}</Link></strong><span>{execution.cancelRequestedAt && execution.status !== "CANCELLED" ? "CANCELAMENTO SOLICITADO" : execution.status}</span><em>{execution.pullRequest ? <a href={execution.pullRequest.url} target="_blank" rel="noreferrer">PR #{execution.pullRequest.externalNumber}</a> : execution.model ?? "—"}</em>{role === "MANAGER" && execution.status === "WAITING_APPROVAL" && !execution.cancelRequestedAt && <OpenPullRequestButton executionId={execution.id} />}{role === "MANAGER" && ["QUEUED", "PREPARING", "RUNNING", "VALIDATING", "WAITING_APPROVAL"].includes(execution.status) && !execution.cancelRequestedAt && <CancelExecutionButton executionId={execution.id} />}{execution.summary && <p>{execution.summary}</p>}{error && <p className="execution-error"><strong>{error.title}</strong><span>{error.action}</span></p>}</div>; })}{!demand.executions.length && <div className="list-empty">A execução ficará disponível após a aprovação da demanda.</div>}</div></section>
      </div>
    </AppShell>
  );
}
