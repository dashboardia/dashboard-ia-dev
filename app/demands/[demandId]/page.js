import { ArrowRight, CheckCircle2, Clock3, GitBranch, MousePointerClick, User } from "lucide-react";
...
export default async function DemandPage({ params }) {
  ...
  const demand = await db.demand.findUnique({
    where: { id: demandId },
    include: { project: { include: { createdBy: { select: { globalRole: true, billingAccount: { select: { plan: true, planDefinition: { select: { priceCents: true, includedCredits: true } } } } } } } }, createdBy: { select: { id: true, name: true, githubLogin: true } }, approvedBy: { select: { name: true, githubLogin: true } }, executions: { include: { pullRequest: true }, orderBy: { createdAt: "desc" } } },
  });
  if (!demand) notFound();
  ...
  return (
    <AppShell user={user}>
      <div className="section-page">
        <SectionHeader backHref="/demands" eyebrow={`${demand.project.name} · ${typeLabels[demand.type]}`} title={demand.title} description={`Demanda ${demand.id.slice(-10)} · criada por ${demand.createdBy.name ?? demand.createdBy.githubLogin}`} action={role === "MANAGER" && demand.status === "PENDING_APPROVAL" ? <ApproveButton demandId={demand.id} /> : role === "MANAGER" && ["APPROVED", "FAILED", "STOPPED"].includes(demand.status) ? <StartAnalysisButton demandId={demand.id} /> : null} />
        <div className="detail-grid demand-detail-grid">
          <DemandEditCard demand={{ id: demand.id, title: demand.title, description: demand.description, acceptanceCriteria: demand.acceptanceCriteria, type: demand.type, priority: demand.priority, visualValidation: demand.visualValidation, visualPaths: demand.visualPaths, aiModel: demand.aiModel }} canEdit={canEdit} lunaOnly={lunaOnly} />
