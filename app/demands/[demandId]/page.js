  ...
  return (
    <AppShell user={user}>
      <div className="section-page">
        ...
        <section className="form-card detail-card full-card"><div className="card-heading"><div><h2>Execuções</h2><p>Histórico imutável das tentativas</p></div></div><div className="simple-list">{demand.executions.map((execution) => <div className="execution-entry" key={execution.id}><strong>{execution.stage}</strong><span>{execution.status}</span><em>{execution.pullRequest ? <a href={execution.pullRequest.url} target="_blank" rel="noreferrer">PR #{execution.pullRequest.externalNumber}</a> : execution.model ?? "—"}</em>{role === "MANAGER" && execution.status === "WAITING_APPROVAL" && <OpenPullRequestButton executionId={execution.id} />}{execution.summary && <p>{execution.summary}</p>}{execution.error && <p className="execution-error">{execution.error}</p>}</div>)}{!demand.executions.length && <div className="list-empty">A execução ficará disponível após a aprovação da demanda.</div>}</div></section>
      </div>
    </AppShell>
  );
}
