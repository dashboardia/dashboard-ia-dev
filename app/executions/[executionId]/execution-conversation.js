...
export default function ExecutionConversation({ executionId, status, messages, expiresAt, adjustmentCount, maxAdjustments }) {
  ...
  return <section className="form-card detail-card full-card execution-conversation">
    ...
    {available && <form className="execution-reply-form" onSubmit={sendAdjustment}><label htmlFor="execution-adjustment">Novo ajuste</label><textarea id="execution-adjustment" value={content} onChange={(event) => setContent(event.target.value)} placeholder="Explique o que precisa mudar ou acrescente um novo contexto..." minLength={3} maxLength={12000} required /><div className="execution-conversation-actions"><small>{adjustmentCount}/{maxAdjustments} ajustes usados · cobrança pelo uso medido · disponível até {new Date(expiresAt).toLocaleString("pt-BR")}</small><div><button className="execution-complete-button" type="button" onClick={completeExecution} disabled={loading}><CheckCircle2 size={16} />Concluir</button><button className="primary compact" type="submit" disabled={loading}>{loading ? <LoaderCircle className="spin" size={16} /> : <Send size={16} />}{loading ? "Enviando..." : "Enviar ajuste"}</button></div></div></form>}
